...
def KNN(fever_,tiredness_,cough_,difficulty_in_breathing_,sneeze_):
    ...
    health_index=score_finder(l_score)
...
def prescription():
    ...
    for i in l:
        inp=int(input(i+":"))
        l_scores.append(inp)
    l_future=KNN(l_scores[0],l_scores[1],l_scores[2],l_scores[3],l_scores[4])
    for i in range(3):
        l_prescriptions=[]
        if l_scores[1]>l_future[i][1]:
            if score_finder(l_future[i])<=score_finder(l_scores):
                l_prescriptions.append("ورزش های سبک بکنید")
        elif score_finder(l_future[i])<200:
            l_prescriptions.append("ورزش های هوازی بکنید")
        else:
            l_prescriptions.append("استراحت بکنید")
        if l_future[i][4]>1:
            l_prescriptions.append("غذای چرب نخورید")
        if l_future[i][0]>38:
            l_prescriptions.append("مایعات بنوشید")
        if l_future[i][2]>2:
            l_prescriptions.append("مایعات گرم بنوشید")
        l_final.append(l_prescriptions)
    for i in range(1,4):
        TTS("برای روز "+str(i),"fa")
        for item in l_final[i-1]:
            TTS(item,"fa")
prescription()
